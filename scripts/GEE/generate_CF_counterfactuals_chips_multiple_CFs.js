...
//just for testing
CFs = ee.FeatureCollection(CFs.toList(CFs.size()).slice(0,2)); 
var place = 'Cambodia'; 
...
//////////////////////////////////////////////////////////////////////////////////////////////////
print(CFs,'CFs')
function run_all_chips(cf){
// var cfs_w_chips = CFs.map(function(cf){
  //create chips inside a CF boundary from raster pixels 
  var img_chips = ee.FeatureCollection(generate_img_chips(cf.geometry(),resolution).get(0));
  // var centroids = ee.FeatureCollection(generate_img_chips(CF_example,resolution).get(1));
  //get a random selection of the image_chips? decide if this should be included or not 
  img_chips = ee.FeatureCollection(img_chips.toList(img_chips.size()).shuffle().slice(0,ee.Number(img_chips.size()).divide(2).round()));
  ////////////////////////////////////////////////////////////////////////////////
  ...
  img_chips = img_chips.map(function(feat){
    ...
    return feat.set('canopyCover',cc_out)
               ...
               .set('source_CF',cf.get('CF_Name_En')); //this will have to be changed to run all CFs
  }); 
  return img_chips
}
var cfs_w_chips = CFs.map(run_all_chips)
print(cfs_w_chips,'chips')
...
function mask_all_cfs(fc){
  var masked_chips = ee.FeatureCollection(fc).map(function(feat){
    ...
    vect_chips = vect_chips.map(function(f){
      return f.set('source_chip_id',feat.get('id')).set('source_CF',feat.get('source_CF')); 
    }); 
    return vect_chips;
  }); 
  return masked_chips.flatten()
}
var all_analogue_chips = ee.FeatureCollection(cfs_w_chips.flatten().map(mask_all_cfs)).flatten()
// print(all_analogue_chips)
// Map.addLayer(all_analogue_chips)

//remove chips that overlap other CFs
var filterInside = ee.Filter.bounds(CFs);
var filterNot = filterInside.not();
all_analogue_chips = all_analogue_chips.filter(filterNot); 

//add the actual predictor variable values back into the features as properties so we can see and plot them 
all_analogue_chips = all_analogue_chips.map(function(feat){
  var cc_out = get_aoi_stat(canopyCover.select(baselineYr),feat,baselineYr,'canopyCover',ee.Reducer.mean()); 
  //this could be amended to be distance from CF centroid to road 
  var rd_out = get_aoi_stat(road_dist,feat,'distance','dist_to_rd',ee.Reducer.mean()); 
  var edge_out = get_aoi_stat(dist_to_edge,feat,'distance','dist_to_edge',ee.Reducer.mean());
  ...
  return feat.set('canopyCover',cc_out)
             ...
             .set('aspect',aspect_out)
             .set('density',density_out)
             .set('source_CF',feat.get('source_CF')); //this will have to be changed to run all CFs
}); 
...
Export.table.toAsset({
  collection:all_analogue_chips,
  description:'image_chip_counterfactual_testing_multiple_CFs',
  assetId:'general_exports'+'/image_chip_counterfactual_testing_multiple_CFs'
}); 

Export.table.toDrive({
  collection: all_analogue_chips,
  description:'CF_image_chips_from_rasters_multiple_CFs', 
  fileNamePrefix:'CF_image_chips_from_rasters_multiple_CFs',
